...
code, managing project versions, downloading library dependencies, etc. This
book uses both tools, each one where it is the most convenient. The compiler
\token{gyc} is invoked directly for the short single file examples that make up
most of the listings, and whenever the intermediate representations of the
compiler are examined, as in the previous section. Gyllir is used as soon as a
program is made of several source files, or depends on an external
library. Gyllir can initialize a new project with the
