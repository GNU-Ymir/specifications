...
(a scope being defined by the tokens \token{\{\}}). Spaces surround the
operators, and parameter operators such as \token{()} and \token{[]} are always
spaced as follows \token{foo (a, b, c)}.
