...
represented by the flow graph presented in Figure~\ref{fig:flow:if_cond_simple}, or
by the YIL code in Listing~\ref{lst:flow:if_cond_simple}.
...
assignment to \token{b} moves borrowed mutable data.
...
We have seen in Section~\ref{sec:global:function_early_return} that a function can exit
...
therefore has no value. Section~\ref{sec:flow:inf_loop} presents a construction
...
In the example presented in Listing~\ref{lst:flow:if_cond_early_return}, the value of
...
case. Figure~\ref{fig:flow:if_cond_early_exit} shows the flow graph resulting from
...
We have seen in Section~\ref{sec:memory:pattern_vdecl} that variables can be declared
...
Figure~\ref{fig:flow:if_cond_let_Ok}, as you can see the variable \token{x} is
...
As the presentation of patterns shows in
...
Listing~\ref{lst:flow:using_second_cond_no_guard}, the pattern guard allows the
