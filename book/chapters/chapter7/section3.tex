...
type (cf. Section~\ref{sec:flow:cond_value_type}). The control flow of the next
Listing~\ref{lst:flow:simple_loop_break} is illustrated in
Figure~\ref{fig:flow:simple_loop_break}.

\begin{lstlisting}[style=coloredverbatim, label=lst:flow:simple_loop_break, caption=Breaking a loop with a value]
let mut count = 0;
