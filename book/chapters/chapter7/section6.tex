...
Section~\ref{sec:class_override_lst_compr}.
...
loops as shown in the next Listing~\ref{lst:flow:compr_two_loops}.
...
\begin{lstlisting}[style=coloredverbatim, caption=Using two loops, label=lst:flow:compr_two_loops]
let dmut a = dcopy [[1, 2], [3, 4]];
...
Listings~\ref{lst:flow:list_compr_tuple_iter} and~\ref{lst:flow:list_compr_tuple_create}
produce the YIL (Ymir Intermediate Language) representation shown respectively
in Listings~\ref{lst:flow:list_compr_tuple_iter_yil}
and~\ref{lst:flow:list_compr_tuple_create_yil}.

\begin{lstlisting}[style=coloredverbatim, label=lst:flow:list_compr_tuple_iter, caption=List comprehension iterating over a tuple]
let a = (1, 2, 3);
let b = [i for i in a];
\end{lstlisting}

\begin{lstlisting}[style=lyilVerb, label=lst:flow:list_compr_tuple_iter_yil, caption=YIL representation of Listing~\ref{lst:flow:list_compr_tuple_iter}]
a(#2) = (1, 2, 3);
i(#3) = a(#2)._0;
i(#5) = a(#2)._1;
i(#7) = a(#2)._2;
...
\begin{lstlisting}[style=coloredverbatim, label=lst:flow:list_compr_tuple_create, caption=List comprehension iterating over a tuple]
let a = [1, 2, 3];
let b = (i for i in a,);
\end{lstlisting}

\begin{lstlisting}[style=lyilVerb, label=lst:flow:list_compr_tuple_create_yil, caption=YIL representation of Listing~\ref{lst:flow:list_compr_tuple_create}]
a(#1) = [1, 2, 3];
YI(#3) = (len-> 3, ptr-> &a(#2));
#_value(#4) = YI(#3);
#_iter(#5) = 0;
...
The following Listings~\ref{lst:flow:list_compr_range_iter}
and~\ref{lst:flow:list_compr_array_iter} produce the YIL (Ymir Intermediate Language)
representation shown respectively in
Listings~\ref{lst:flow:list_compr_range_iter_yil}
and~\ref{lst:flow:list_compr_array_iter_yil}.

\begin{lstlisting}[style=coloredverbatim, label=lst:flow:list_compr_range_iter, caption=List comprehension iterating over a range]
let a = [i for i in 0 .. 10];
\end{lstlisting}

\begin{lstlisting}[style=lyilVerb, label=lst:flow:list_compr_range_iter_yil, caption=YIL representation of Listing~\ref{lst:flow:list_compr_range_iter}]
    #_iter(#1) = 0;
#GOTO test(#3);
#LABEL beg(#2);
...
\begin{lstlisting}[style=coloredverbatim, label=lst:flow:list_compr_array_iter, caption=List comprehension iterating over an array]
let mut a = [1, 2, 3];
let b = [i for i in a];
\end{lstlisting}

\begin{lstlisting}[style=lyilVerb, label=lst:flow:list_compr_array_iter_yil, caption=YIL representation of Listing~\ref{lst:flow:list_compr_array_iter}]
    a(#2) = [1, 2, 3];
...
One can note from Listings~\ref{lst:flow:list_compr_tuple_iter_yil}
and~\ref{lst:flow:list_compr_array_iter_yil} that the iteration over an array value
...
As discussed in Section~\ref{sec:compound:slice_expansion}, slices can be expanded when
...
\begin{lstlisting}[style=coloredverbatim, label=lst:flow:list_compr_slice_to_array, caption=List comprehension over a slice with cte size]
fn foo()-> [i32];
...
In the above example, the \token{copy} operator creates a slice value
