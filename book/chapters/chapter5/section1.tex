...
file hierarchy shown in Figure~\ref{fig:global:file_hierarchy}, the module located in
...
Figure~\ref{fig:global:file_hierarchy}.
...
%% check: skip -- module example needs a real foo/ directory on disk
...
line compiles the package. The \token{-fdump-ymir} option asks it to
...
pub foo
    ...
    foo::qux
        foo::qux::prvFuncInQux
        pub foo::qux::pubFuncInQux
\end{lstlisting}
...
\label{sec:global:symbol_protection}
...
of symbols are visible as described in Figure~\ref{fig:global:symbol_privacy}. In
...
%% check: skip -- refers to the /qux/ package, imported with -I
...
\textit{-c} creates an object file (\textit{qux.o}), that is used
...
them. In \token{foo::bar::tryAccessToBaz} above, for example, the symbol
\token{qux::baz::funcInBaz} was written in full. This can be cumbersome, so the
...
modules used within a given symbol, for every symbol and not only for modules
(the core list is elided with \token{...} below, on the two symbols that add
nothing to it). The core modules are the modules that are
...
pub foo - use {core::exception, core::exception::assertion,
               core::exception::io, core::exception::option,
               core::types}
    foo::bar - use {qux::baz, std::io,
                    core::exception, core::exception::assertion,
                    core::exception::io, core::exception::option,
                    core::types}
        pub foo::bar::tryAccessToBaz - use {core::exception, ...}
    foo::funcInFoo - use {core::exception, ...}
\end{lstlisting}
...
\begin{lstlisting}[caption=\textit{./foo/bar.yr}, style=coloredverbatimError, escapechar=@]
in bar;
