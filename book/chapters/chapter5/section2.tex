...
\label{sec:global:function_parameters}
...
parameters is limited to the scope of the function body. Parameters are
...
Section~\ref{sec:memory:unused_variables}. While it may initially seem unnecessary,
...
by commas. A function argument is a value passed either positionally or by
...
%% check: skip -- /foo/ is the function declared above
\begin{lstlisting}[style=coloredverbatim]
...
The result of all the above calls is the same, and produces the following Ymir
...
\warningbox{ the arguments of the function are constructed in the order of
  ...
  \token{baz}. Even though the order of construction of the arguments is defined
  and predictable, it is not a good habit to count on it, and the use of intermediate
  variables is strongly recommended to avoid any errors.
}
...
foo(a-> bar(), b-> baz());
foo(b-> baz(), bar());
// if the three calls perform the same operations in the same order, make it clear

let x = bar();
let y = baz(); // clearer that bar is called before baz
foo(a-> x, b-> y);
\end{lstlisting}
\smallskip
...
An optional parameter can be constructed by a function, and it can even be
...
\begin{lstlisting}[style=coloredverbatimError, escapechar=@]
fn foo(a: i32 = @\hb{foo()}@)-> i32 { // error, /a/ is needed to call /foo/
  a + 1
}

fn bar(a: i32 = @\hb{baz()}@)-> i32 { // error, /a/ is needed to call /baz/
  a + 1
}

fn baz(a: i32 = @\hb{bar()}@)-> i32 { // error, /a/ is needed to call /bar/
  a - 1
}
\end{lstlisting}
...
\begin{lstlisting}[style=coloredverbatim]
// setting the optional parameter a in baz, to stop the infinite recursion
fn bar(a: i32 = baz(a-> 1))-> i32 {
  a + 1
}

// setting the optional parameter a in bar, to stop the infinite recursion
...
The above source code produces the following YIL result.
...
\label{sec:global:function_body}

A function body is an expression that describes a value. There are several types
...
A function whose prototype is followed by a semicolon instead of a body declares
a function that is defined somewhere else. The prototype is all the compiler
needs to validate the calls to it; the actual code of the function is provided
at link time, by an object file passed on the command line. This is the same
mechanism as the external declarations presented in
Section~\ref{sec:global:external_decls}, applied to a symbol that keeps the Ymir
mangling of the module declaring it.

Let's suppose the two packages \token{foo} and \token{bar}, where \token{bar}
declares a public function \token{funcInBar} that the package \token{foo}
calls. The declaration of \token{funcInBar} reaches \token{foo} through the
\token{-I} option, as any package importation does
(cf. Section~\ref{sec:global:modules}), and its code through the object file
\token{libbar.o}.

\begin{minipage}[t][][t]{0.47\linewidth}
\begin{lstlisting}[caption=\textit{./bar.yr}, style=coloredverbatim]
in bar;
use std::io;
...
\begin{minipage}[t][][t]{0.47\linewidth}
%% check: skip -- /bar/ is the package imported with -I below
...
@+\textcolor{teal!80}{alice@dev:\mtilde}@+$ gyc -c bar.yr -o libbar.o
@+\textcolor{teal!80}{alice@dev:\mtilde}@+$ gyc foo.yr -I bar.yr libbar.o -o foo.exe
...
A bodiless prototype is what makes this useful: the declarations that \token{foo}
imports are never validated (cf. Section~\ref{sec:global:modules}), so the file passed
to \token{-I} only has to declare the prototypes of the symbols that are called.
A package can therefore be distributed as a set of prototypes plus a binary,
rather than as full source code, in the same spirit as the header files that
could be found in C/C++ languages. The rules of Section~\ref{sec:global:modules} still
apply to that file: it must be named after the module it declares, so it is
placed in a directory of its own.

\begin{lstlisting}[caption=\textit{./include/bar.yr}, style=coloredverbatim]
in bar;

pub fn funcInBar(a: i32)-> i32; // defined elsewhere, linked in
\end{lstlisting}

\begin{lstlisting}[style=bashVerb, escapechar=@+]
@+\textcolor{teal!80}{alice@dev:\mtilde}@+$ gyc foo.yr -I include/bar.yr libbar.o -o foo.exe
\end{lstlisting}
...
The keyword \token{return} returns early. It is associated with a
...
The value of the function is retrieved by the caller of the function, as the
...
\label{sec:global:function_early_return}
...
statements and expressions that are not reachable because they are written
...
A function can exit through various methods, returning normally and taking the
...
exit makes the function unsafe as it leaves the function without a value, and
...
body of the function the statement \token{throw V;}, with \token{V} an
...
{
  println("In foo.");
  if (!a) throw copy AssertError("a must be true."); // exit the function
...
When calling the function \token{foo}, thanks to that information, it becomes
apparent inside the caller that \token{foo} is throwing an exception and might
return without a value. The compiler enforces the caller function to take that
into account either by catching the exception, and performing other operations in
case of the failure of the function \token{foo} (this will be treated in
Chapter~\ref{chap:flow}), or by enforcing the caller function to also
define in its prototype that exception might be thrown.

%% check: skip -- /foo/ is the throwing function declared above
\begin{lstlisting}[style=coloredverbatim]
fn bar()
   throws AssertError
{
...
Exceptions might not only exit the function that is throwing them, but
...
program might throw an exception. When no exceptions are displayed in the
...
%% check: skip -- /bar/ is the throwing function declared above
\begin{lstlisting}[style=coloredverbatim]
fn main() {
  {
    bar();
...
Chapter~\ref{chap:flow}, which also covers the close
...
Hence, the \token{baz} function is actually unsafe. There is not much we can do
...
caught by any function. It can also be unsafe. In the next example, the
\token{main} function declares two exception types: \token{AssertError}, which
it throws itself, and \token{CastFailure}, which the conversion \token{to} may
throw and which \token{main} does not catch.

\begin{lstlisting}[style=coloredverbatim, caption=The most complex main function prototype]
use std::{io, conv, conv::errors}; // for string to int conversion
   ...
   throws AssertError, CastFailure
{
  ...
  if (args.len != 2us) throw copy AssertError("Missing or wrong number of parameters");

  let mut a = args[1].to!{i32}();
  let dmut b = &a;
...
\notebox{
  ...
  level of a file, but that was just for brevity. All control flow is described
