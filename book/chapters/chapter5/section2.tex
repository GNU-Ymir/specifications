...
Section~\ref{sec:external_decls}, applied to a symbol that keeps the Ymir
...
(cf. Section~\ref{sec:modules}), and its code through the object file
\token{libbar.o}.
...
imports are never validated (cf. Section~\ref{sec:modules}), so the file passed
...
could be found in C/C++ languages. The rules of Section~\ref{sec:modules} still
