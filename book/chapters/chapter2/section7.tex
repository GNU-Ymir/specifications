...
represented by the keyword \token{bool}, and is encoded in one byte of memory
...
(also called disjunction), denoted as $\lor$ in Boolean algebra and written as
...
Boolean algebra defines secondary operators, that are formed from the basic
...
A good example of a possible error is to rely on commutativity alone while forgetting about precedence, leading to differences in associativity. In the next listing the variables \token{r1} and \token{r2} will hold different values, due to the laws we have described.
...
Consequently even though standard boolean algebra specifies that AND and OR
operators are commutative, they are not in Ymir.
