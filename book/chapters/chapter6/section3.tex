...
\label{sec:memory:variable_mutability}

A variable is immutable by default, but can be made mutable using the
\token{mut} and \token{dmut} keywords. We saw in
Section~\ref{sec:compound:preamble_compound_types} that values can borrow data depending
...
\warningbox{
  ...
  Sections~\ref{sec:memory:ref_variable} and~\ref{sec:memory:variable_laziness}.
}
...
Figure~\ref{fig:memory:example_mut_params} shows the stack and heap memory state when
...
it using the keyword \token{alias}. This keyword indicates that the
...
keyword is unnecessary, and not allowed. Avoid it for
