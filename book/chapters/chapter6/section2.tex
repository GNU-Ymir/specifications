...
\label{sec:memory:local_variable_declaration}
...
Section~\ref{sec:memory:variable_lazy_mut}. The identifier of a variable follows the
same syntax as any identifier in Ymir. The type part of the variable declaration
is optional (cf. Section~\ref{sec:memory:variable_type}), and its syntax is any
expression that can describe a type. The value part of the declaration is
mandatory for a global variable (cf. Section~\ref{sec:memory:variable_init_value}), and
its syntax is any expression that can describe a value.

\begin{lstlisting}[style=bashVerb]
global_variable := 'lazy' ('mut' | 'dmut')? Identifier (':' type)?
                   '=' value ';'
\end{lstlisting}
...
Section~\ref{sec:memory:variable_type}), and its syntax is any expression that can
describe a type. The value part of the declaration is optional for a parameter
variable (cf. Section~\ref{sec:memory:variable_init_value}), and its syntax is any
...
Section~\ref{sec:global:symbol_protection}.
...
As presented in Section~\ref{sec:global:function_body}, a scope has a value if the last
...
\label{sec:memory:variable_init_value}
...
\begin{minipage}[t][][t]{0.47\linewidth}
  \begin{lstlisting}[style=coloredverbatimError, escapechar=@, caption=Invalid]
fn foo(cond: bool) {
  let i: i32;
  if cond {
    i = 42;
...
Section~\ref{sec:memory:variable_laziness}.

\subsection{Unused variable}
\label{sec:memory:unused_variables}
...
symbol with public access (cf. Section~\ref{sec:global:symbol_protection}), including
...
\label{sec:memory:variable_type}
...
lvalue that will receive it during assignment. Other types such as \token{void}
