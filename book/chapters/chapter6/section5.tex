\section{Laziness}
\label{sec:memory:variable_laziness}
...
let A(x-> @\hb{lazy}@ x) = lazy copy A(); // error, lazy cannot decorate a variable in a pattern matching
...
glance. The output of the program depicted in Listing~\ref{lst:memory:accept_laziness}
is presented in Listing~\ref{lst:memory:result_accept_laziness}, from that result one
...
\begin{lstlisting}[style=coloredverbatim, label=lst:memory:accept_laziness, caption=Example of laziness contract acceptance]
fn foo(useA: bool, lazy a: i32)-> i32 {
  println("In foo");
  let x = if (useA) {
    a
...
\begin{lstlisting}[style=bashVerb, caption=Result of execution of Listing~\ref{lst:memory:accept_laziness}, label=lst:memory:result_accept_laziness]
In foo
In Bar
Exit foo
a = 12
...
\label{sec:memory:variable_lazy_mut}
