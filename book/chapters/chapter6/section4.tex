...
\label{sec:memory:ref_variable}

The \token{ref} keyword decorates a variable when constructing it
...
code, and Figure~\ref{fig:memory:ref_variable} illustrates the state of the stack when the
...
\label{sec:memory:ref_param}

The \token{ref} keyword also decorates a parameter variable, and there it passes
the value by reference. To pass a value by
...
In the Listing~\ref{lst:memory:result_copy_v_ref_array}, the value is not copied when passed to the \token{foo}
...
Figure~\ref{fig:memory:example_call_ref_array} and
Figure~\ref{fig:memory:example_call_copy_array} show the state of the stack when
...
\begin{lstlisting}[style=coloredverbatim, label=lst:memory:result_copy_v_ref_array, caption=Example of passing an array by reference vs. by value]
fn foo(ref x: [i32 ; 10]) {
  println(x[5]);
}
...
\begin{lstlisting}[style=lyilVerb, caption=YIL result of Listing~\ref{lst:memory:result_copy_v_ref_array}]
...
\label{sec:memory:mut_ref_param}
