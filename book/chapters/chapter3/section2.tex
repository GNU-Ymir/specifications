...
\begin{center}
  \begin{tabular}{|c|lll|}
    ...
    \hline
    \hline
    \token{f32} & 32 & 8 & 23\\[0pt]
    \token{f64} & 64 & 11 & 52\\[0pt]
    \token{f80} & 80 & 15 & 64\\[0pt]
    \token{fsize} & arch & arch & arch\\[0pt]
    \hline
  \end{tabular}
\end{center}
...
scientific notation, and hexadecimal notation.

\begin{enumerate}
  \setlength\itemsep{-4pt}
\item Decimal, two decimal int literals separated by the token \token{.} (with
  no spaces in between), e.g. \token{1837.0289}. Both parts are mandatory,
  neither can be omitted (e.g. \token{10.0} is valid, but neither \token{10.}
  nor \token{.10} are).
...
\item Hexadecimal notation, starting with \token{0x}, then two hexadecimal int
  literals separated by the \token{.} token, followed by the letter \token{p}
  ...
  \begin{adjustbox}{max width=1.01\linewidth}
    \begin{threeparttable}
      \begin{tabular}{|l|ll|}
        ...
        \hline
        \hline
        \token{init} & The initial value - nan (Not a Number) & \token{typeof(x)}\\[0pt]
        \Xhline{0.001pt}
        \token{max} & The maximal finite value & \token{typeof(x)}\\
        & that this type can encode & \\[0pt]
        \Xhline{0.001pt}

        \token{min} & The smallest positive normalized & \token{typeof(x)}\\
        & value that this type can encode & \\[0pt]
        \Xhline{0.001pt}

        \token{nan} & The value Not a Number & \token{typeof(x)}\\[0pt]
        \Xhline{0.001pt}
        \token{inf} & The value positive infinity & \token{typeof(x)}\\[0pt]
        \Xhline{0.001pt}
        \token{epsilon} & The smallest increment & \token{typeof(x)}\\
        & to the value 1 &\\[0pt]
        \Xhline{0.001pt}
        \token{dig} & The number of decimal digits of precision & \token{u32}\\[0pt]
        \Xhline{0.001pt}
        \token{mant_dig} & Number of bits in the mantissa & \token{u32}\\[0pt]
        \Xhline{0.001pt}
        \token{max_10_exp} & The maximum value &  \token{i32} \\
        & such that \(10^{max\_10\_exp}\) is representable &\\[0pt]
        \Xhline{0.001pt}
        \token{max_exp} & The maximum value & \token{i32}\\
        & such that \(2^{max\_exp-1}\) is representable & \\[0pt]
        \Xhline{0.001pt}
        \token{min_10_exp} & The minimum value & \token{i32}\\
        & such that \(10^{min\_10\_exp}\) is representable$^1$  & \\[0pt]
        \Xhline{0.001pt}
        \token{min_exp} & The minimum value & \token{i32}\\
...
\cautionbox{ The \token{min} value is not the opposite of the \token{max}
  value. Figure~\ref{fig:scalars:float_point_size} describes the ordering relationship
...
\item To integer types: The cast operator converts a float value
  ...
  value is truncated towards zero (e.g. \token{1.3}, \token{1.5} and
  \token{1.8} are all truncated to \token{1}, and \token{-1.8} to \token{-1}),
  and the fractional part of the float is lost. The reverse cast is also allowed (from
...
Aside from these two cases, floating point values cannot be cast to any other
type.
...
let a: f32 = 1.0; // ok, f64 is converted implicitly to f32
...
The \token{-} unary operator can be used on floating point types. The result
...
operand). The binary operators are divided into four groups:
  ...
  \begin{center}
    \begin{adjustbox}{max width=1.0\linewidth}
      \begin{tabular}{|c|lll|}
        ...
        \hline
        \hline
        \token{+} & Addition & Yes & \token{1.0 + 2.3 == 3.3}\\[0pt]
        \token{-} & Subtraction & No & \token{1.0 - 8.0 == -7.0}\\[0pt]
        \token{*} & Multiplication & Yes & \token{3.0 * 4.0 == 12.0}\\[0pt]
        \token{/} & Division & No & \token{7.0 / 3.0 <=> 2.333}\\[0pt]
        \tokennolst{\%} & Remainder of division & No & \tokennolst{7.23 \% 3.09 <=> 1.05}\\[0pt]
        \tokennolst{\textasciicircum{}\textasciicircum{}} & Exponent & No & \tokennolst{7.0 \textasciicircum{}\textasciicircum{} 3 == 343.0}\\[0pt]
        \hline
      \end{tabular}
    \end{adjustbox}
  \end{center}
...
\item Comparisons: Binary comparison operators can be used with two float values. The
  largest type of the two operands is the one that casts the value of the operand with
  ...
  \begin{center}
    \begin{adjustbox}{max width=1.0\linewidth}
      \begin{tabular}{|c|lll|}
        ...
        \hline
        \hline
        \token{>} & Greater than & No & \token{(12.0 > 11.0) == true}\\[0pt]
        \token{<} & Lower than & No & \token{(12.0 < 11.0) == false}\\[0pt]
        \token{>=} & Greater or equal & No & \token{(14.0 >= 14.0) == true}\\[0pt]
        \token{<=} & Lower or equal & No & \token{(11.0 <= 19.0) == true}\\[0pt]
        \token{==} & Equal & Yes & \token{(10.0 == 10.0) == true}\\[0pt]
        \token{!=} & Not equal & Yes & \token{(10.0 != 10.0) == false}\\[0pt]
        \token{<=>} & Is congruent & Yes & \token{(10.0 <=> 10.0) == true}\\[0pt]
        \token{<!>} & Is not congruent & Yes & \token{(10.0 <!> 10.0) == false}\\[0pt]
        \hline
      \end{tabular}
    \end{adjustbox}
  \end{center}
   ...
   to evaluate whether the left and right operands are equal to within that
   epsilon. They also check for \token{nan} equality and inequality.
...
\item Assignment: The assignment operator \token{=} can be used when the two
  ...
  assignment operator can be mixed with an arithmetic operator (e.g.
  \token{+=}, \token{/=}, etc.). In this case, the operation \token{x += y}
  is rewritten as \token{x = x + (y)}, where the y operand always has a higher
  priority than the assignment operator. For example, the operation \token{x
    *= 12.0 + 3.0} is rewritten as \token{x = x * (12.0 + 3.0)}, even though the
  multiplication operator has a higher priority than the addition operator, i.e.
  the result of \token{x *= (12.0 + 3.0)} is different from the result of
  \token{x = (x * 12.0 + 3.0)}.
...
\item Range: The range operator can be used on two float values of exactly the
  ...
  Section~\ref{sec:compound:range_type}).
  ...
  \begin{center}
    \begin{adjustbox}{max width=1.0\linewidth}
      \begin{tabular}{|l|lll|}
        ...
        \hline
        \hline
        \token{..} & Range operator not inclusive & \token{34.0f .. 12.0f} & \token{[34.0f;12.0f[}\\[0pt]
            \token{...} & Range operator inclusive & \token{5.0f ... 89.0f} & \token{[5.0f;89.0f]}\\[0pt]
            \hline
      \end{tabular}
    \end{adjustbox}
  \end{center}
