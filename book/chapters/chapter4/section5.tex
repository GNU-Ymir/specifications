...
of the array borrows data, similar to tuples. Figure~\ref{fig:(chap4):data_repr_array}
...
\begin{center}\begin{adjustbox}{max width=\linewidth}
  \begin{tabular}{|l|ll|}
    ...
    \hline
    \hline
    \texttt{init} & The initial value, where all inner & \texttt{typeof(x)} \\
    & values are set to \texttt{T::init}, \texttt{T} being & \\
...
\item Concatenation: The concatenation operator \token{\~} is used to create an
  ...
  discussed further in Section~\ref{sec:slices}.
...
Section~\ref{sec:slices}). For big string literals, it has to be taken into
account.
