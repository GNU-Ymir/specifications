  ...
  \begin{center}\begin{adjustbox}{max width=1.0\linewidth}
      \begin{threeparttable}
        \begin{tabular}{|c|ll|}
          ...
          \hline
          \hline
          \texttt{hasValue} & true iff the option has a value & \texttt{bool} \\
          \texttt{hasError} & true iff the option has no value but has an error & \texttt{bool} \\
          \Xhline{0.001pt}
          \texttt{value} & the value contained in the option & \textit{T}$^{1^{\phantom{j}}}$ \\
          \texttt{error} & the error contained in the option & \textit{\&Exception}$^{2^{\phantom{j}}}$ \\
          \hline
        \end{tabular}
        \begin{tablenotes}
  ...
  \begin{center}\begin{adjustbox}{max width=1\linewidth}
      \begin{threeparttable}
        \begin{tabular}{|c|lll|}
          ...
          \texttt{>=} & Greater or equal & No & \texttt{((12)? >= (11)?) == true}\\
          \texttt{<=} & Lower or equal & No & \texttt{((12)? <= (11)?) == false}\\
          \hline
        \end{tabular}
      \end{threeparttable}
  \end{adjustbox}\end{center}
...
casting (see Section~\ref{sec:slices}), the opposite is not possible, for the
same reason. This
